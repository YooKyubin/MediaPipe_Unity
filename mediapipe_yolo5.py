...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        for i in range(len(resultList)):
            xmi, ymi, xma, yma, _, _ = map(int, resultList[i])
            if xma-xmi >= 100 and yma-ymi >= 400:
                part_data.append([])
        ...
        for i in range(len(resultList)):
            xmi, ymi, xma, yma, _, _ = map(int, resultList[i])
            if xma-xmi >= 100 and yma-ymi >= 400:
                th = threading.Thread(target=holiticProcess, args=(image, resultList[i], holistic[i], part_data, partDataIdx, ))
                th.start()
                threads.append(th)
                partDataIdx += 1
        ...
        cv2.putText(image, fps_str, (5, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2, cv2.LINE_AA)
